...
$stmt_turmas_prepare = mysqli_prepare($conn, $sql_turmas_prof); // Nome da variável ajustado
...
if ($stmt_turmas_prepare) {
    ...
    $result_turmas_assoc = mysqli_stmt_get_result($stmt_turmas_prepare);
    while($turma_row = mysqli_fetch_assoc($result_turmas_assoc)){
        $turmas_professor_result[] = $turma_row;
    }
    mysqli_stmt_close($stmt_turmas_prepare);
} else {
    error_log("Erro ao buscar turmas do professor (lancar_comunicado.php): " . mysqli_error($conn));
}
...
$stmt_comunicados_prepare = mysqli_prepare($conn, $comunicados_enviados_sql); // Nome da variável ajustado
$comunicados_enviados_result_data = null; // Nome da variável ajustado
if($stmt_comunicados_prepare){
    mysqli_stmt_bind_param($stmt_comunicados_prepare, "i", $professor_id);
    mysqli_stmt_execute($stmt_comunicados_prepare);
    $comunicados_enviados_result_data = mysqli_stmt_get_result($stmt_comunicados_prepare);
} else {
     error_log("Erro ao buscar comunicados enviados (lancar_comunicado.php): " . mysqli_error($conn));
}
...
:root {
    ...
    /* Cores de status (manter do original para as mensagens de status) */
...
/* Estilos específicos da página lancar_comunicado.php */
...
.form-section button[type="submit"] {
    padding: 0.85rem 2rem; /* Um pouco maior */
    border: none;
...
.chat-user-coordenador-acad .chat-user-name-acad {
    font-weight: bold;
    font-style: italic;
    color: var(--status-info); /* Usando --status-info para coordenador */
}
...
::-webkit-scrollbar-thumb:hover {
    background: linear-gradient(135deg, var(--primary-color-dark), #145A57);
        ...
        /* Estilos da página lancar_comunicado.php */
        .form-section, .list-section { 
...
<body class="theme-<?php echo htmlspecialchars($tema_global_usuario); ?>"> 
    ...
    <div class="container" id="pageContainer">
        ...
        <main class="main-content">
            <h2 class="page-title">Novo Comunicado</h2>
            ...
            <section class="form-section dashboard-section card">
                <form action="salvar_comunicado.php" method="POST">
                    ...
                    <p style="font-size:0.8em; color: var(--text-color-muted, #666);">Se nenhuma turma for selecionada, o comunicado será considerado geral para os alunos das turmas que você leciona.</p>
            ...
            <section class="list-section dashboard-section card">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php while($comunicado = mysqli_fetch_assoc($comunicados_enviados_result_data)): ?>
                        <tr>
...
<?php 
if(isset($stmt_comunicados_prepare)) mysqli_stmt_close($stmt_comunicados_prepare); // Corrigido nome da variável stmt
